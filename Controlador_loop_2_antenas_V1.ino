...
//polopu 1 uses pins 2 and 5 ( X axis in cnc shield)
//pololu 2 uses pins 3 and 6 ( Y axis in cnc shield)
//low endstop uses pin 4
//up endstop uses pin 7
  ...
  if(digitalRead(UP)==LOW && digitalRead(DOWN)==LOW)   //reset encoderValue si pulsamos pin 17 y 11
              {
                  ...
                  delay(3000); 
                  upperlimit=64000;                                  
        for (i=mempointer; i <= mempointer+94; i++) {
                  EEPROM.put(i,0);     //escribimos el valor o en todas las posiciones
                  }  
                  lcd.clear();
                  encoderValue=1;
